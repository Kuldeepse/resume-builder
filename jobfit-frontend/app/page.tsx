...
import { Sparkles, RefreshCw, BarChart3, AlertTriangle, Target, FileText, User, Code, HelpCircle, RotateCcw, ClipboardCheck, ArrowRight, ArrowLeftRight, CheckCircle2, Sun, Moon, Heart } from 'lucide-react';
    ...
    <main className={`min-h-screen p-4 md:p-8 font-sans antialiased transition-colors duration-300 ${darkMode ? 'bg-stone-950 text-slate-100' : 'bg-[#FAF8F5] text-slate-900'}`}>
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <header className="relative flex flex-col items-center text-center space-y-3 py-4 border-b border-dashed border-amber-900/20 dark:border-slate-800">
          ...
          <h1 className="text-3xl md:text-5xl font-black tracking-tight text-slate-900 dark:text-amber-900">AI Career Intelligence Matrix</h1>
            ...
            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className={`space-y-1.5 p-4 rounded-xl border ${darkMode ? 'bg-stone-950/40 border-slate-800/60' : 'bg-stone-50/60 border-stone-200/60'}`}>
                  ...
                  <textarea rows={6} className={`w-full border p-3 rounded-xl focus:outline-none font-mono text-[11px] leading-relaxed ${darkMode ? 'bg-stone-950 border-slate-800 text-slate-300 focus:border-amber-800' : 'bg-white border-stone-200 text-slate-800 focus:border-amber-900'}`} placeholder="Seeking an engineer with deep runtime comprehension of cloud topologies..." value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} />
                </div>
              </div>
          ...
          <div className={`border p-8 rounded-2xl shadow-md space-y-6 ${darkMode ? 'bg-stone-900/40 border-slate-800/60' : 'bg-white border-amber-900/10'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className={`p-6 rounded-2xl border flex flex-col ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-[#F2ECE4] border-amber-200/80 text-stone-900'}`}>
                ...
                <ul className={`space-y-2.5 text-xs font-semibold leading-relaxed overflow-y-auto pr-1 max-h-48 pl-0 list-none ${darkMode ? 'text-slate-300' : 'text-slate-800'}`}>
              ...
              <div className="space-y-4 pt-3">
                {results.resume?.experience?.map((exp: any, i: number) => (
                  <div key={i} className="space-y-1.5 border-l-2 border-amber-600/50 pl-4 relative">
                    ...
                    <div className="flex justify-between font-bold text-xs"><span>{exp.role} <span className="font-normal opacity-60">at</span> {exp.company}</span><span className="text-indigo-600 dark:text-indigo-400 text-[10px] tracking-wider font-mono">{exp.duration}</span></div>
        ...
        {tab === 'prep' && results && (
          <div className={`border p-6 rounded-2xl shadow-sm space-y-5 ${darkMode ? 'bg-stone-900/40 border-slate-800/60' : 'bg-white border-amber-900/10'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-h-[500px] overflow-y-auto pr-1">
              ...
              <div className="space-y-4">
                <h4 className="text-xxs font-bold uppercase tracking-widest text-indigo-600 dark:text-indigo-400 border-b pb-2 flex items-center gap-1.5 border-amber-900/10 dark:border-slate-800"><Code className="w-4 h-4"/> Domain Execution Vectors</h4>
                {results.technical_interview?.map((item: any, i: number) => (
                  <div key={i} className={`p-4 rounded-xl border space-y-2 transition-all ${darkMode ? 'bg-slate-950 border-slate-800/80 text-slate-100' : 'bg-amber-50/40 border-amber-200 text-slate-900'}`}>
                    <div className="font-bold flex items-start gap-1.5 text-xs"><Code className="w-4 h-4 text-indigo-500 shrink-0 mt-0.5"/> <span>Q: {item.question}</span></div>
                    <div className="text-[11px] pl-5 leading-relaxed opacity-90"><span className="font-bold text-indigo-600 dark:text-indigo-400">Technical Strategy:</span> {item.response}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <footer className="w-full text-center py-6 mt-12 border-t border-dashed border-amber-900/10 dark:border-slate-800">
        <p className="text-[11px] font-bold tracking-widest text-stone-400 dark:text-stone-500 uppercase flex items-center justify-center gap-1.5">
          Crafted with <Heart className="w-3.5 h-3.5 text-rose-600 fill-rose-600 animate-pulse" /> & Developed by <span className="text-amber-900 dark:text-indigo-400 font-extrabold font-mono tracking-normal">Kuldeep Sharma</span>
        </p>
      </footer>
    </main>
  );
