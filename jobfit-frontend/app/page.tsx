  ...
  return (
    <main className={`min-h-screen p-4 md:p-8 font-sans antialiased transition-colors duration-300 ${darkMode ? 'bg-stone-950 text-stone-100' : 'bg-[#FAF8F5] text-stone-900'}`}>
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {results && (
          <div className={`flex flex-wrap border p-1.5 gap-1.5 backdrop-blur-md rounded-2xl shadow-sm overflow-x-auto ${darkMode ? 'bg-stone-900/80 border-stone-800' : 'bg-white border-amber-900/10'}`}>
        ...
        {tab === 'updated_resume' && results && (
          <div className={`border p-6 rounded-2xl shadow-sm space-y-5 ${darkMode ? 'bg-stone-900/40 border-stone-800/60' : 'bg-white border-amber-900/10'}`}>
            ...
            <div className={`border p-6 rounded-xl space-y-4 max-h-[500px] overflow-y-auto shadow-inner leading-relaxed ${darkMode ? 'border-stone-800 bg-stone-950/80' : 'border-stone-200 bg-stone-100/40'}`}>
              <h2 className={`text-xl font-extrabold tracking-tight border-b pb-2 ${darkMode ? 'text-stone-100 border-stone-800' : 'text-stone-900 border-stone-200'}`}>{results.resume?.full_name}</h2>
              <p className={`text-xs font-medium tracking-wide ${darkMode ? 'text-stone-400' : 'text-stone-600'}`}>{results.resume?.professional_summary}</p>
              <div className="flex flex-wrap gap-1.5 pt-1">
                {results.resume?.skills?.map((s: string, i: number) => <span key={i} className={`font-bold px-2 py-0.5 rounded-md border text-[10px] tracking-wide ${darkMode ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-amber-50 text-amber-900 border-amber-200'}`}>{s}</span>)}
              </div>
        ...
        {tab === 'prep' && results && (
          <div className={`border p-6 rounded-2xl shadow-sm space-y-5 ${darkMode ? 'bg-stone-900/40 border-stone-800/60' : 'bg-white border-amber-900/10'}`}>
            <div className={`p-4 rounded-xl text-white flex justify-between items-center shadow-md ${darkMode ? 'bg-stone-950 border border-stone-800' : 'bg-stone-800'}`}>
              <div className="flex items-center gap-1.5 font-bold text-xxs tracking-wider uppercase text-amber-100"><User className="w-4 h-4 text-amber-500"/> Recruitment Readiness Training Matrix</div>
              <button onClick={handleCopyLink} className="bg-amber-900 text-white px-3 py-1.5 rounded-xl font-bold cursor-pointer text-[10px] uppercase tracking-wider transition-all hover:bg-amber-800 shadow-sm border border-amber-800/40">{copied ? "Link Copied!" : "Copy Public PDF Link"}</button>
            </div>
