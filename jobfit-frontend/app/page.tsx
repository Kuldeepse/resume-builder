...
export default function Home() {
  ...
  const handleGenerate = async () => {
    if (!fullName || !targetRole || !careerHistory || !jobDescription) return alert("Please fill out all fields.");
  ...
  return (
    <main className={`min-h-screen p-4 md:p-8 font-sans antialiased transition-colors duration-300 ${darkMode ? 'bg-amber-950 text-amber-50 selection:bg-amber-600' : 'bg-[#FDFBF7] text-stone-900 selection:bg-amber-700'}`}>
      <div className="max-w-5xl mx-auto space-y-6">
        <header className="relative flex flex-col items-center text-center space-y-2 py-4 border-b border-dashed border-amber-200 dark:border-amber-900/60">
          ...
          <h1 className="text-3xl md:text-5xl font-black tracking-tight text-amber-800 dark:text-amber-200">AI Career Intelligence Matrix</h1>
        ...
        {results && (
          <div className={`flex flex-wrap border p-1.5 gap-1.5 backdrop-blur-md rounded-2xl shadow-sm overflow-x-auto ${darkMode ? 'bg-amber-900/20 border-amber-900/60' : 'bg-white border-amber-200'}`}>
            {([['builder', 'Pipeline Builder', Sparkles], ['validation', 'Resume Validation', ClipboardCheck], ['updated_resume', 'Tailored Output', FileText], ['prep', 'Interview Vectors', User]] as const).map(([t, label, Icon]) => (
              <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 font-bold rounded-xl flex items-center gap-1.5 cursor-pointer transition-all duration-200 text-xxs tracking-wide ${tab === t ? 'bg-amber-800 text-white shadow-md' : darkMode ? 'text-amber-300 hover:bg-amber-900/40' : 'text-stone-600 hover:bg-amber-50'}`}><Icon className="w-3.5 h-3.5"/> {label}</button>
            ))}
          </div>
        )}

        {tab === 'builder' && (
          <div className={`border p-6 rounded-2xl shadow-sm space-y-6 ${darkMode ? 'bg-amber-900/10 border-amber-900/40' : 'bg-white border-amber-200'}`}>
            <div className={`flex justify-between items-center border-b pb-3 ${darkMode ? 'border-amber-900/40' : 'border-amber-100'}`}>
              ...
              {results && <button type="button" onClick={() => { setFullName(''); setTargetRole(''); setCareerHistory(''); setJobDescription(''); setResults(null); setTab('builder'); }} className={`flex items-center gap-1 font-bold px-3 py-1 rounded-xl border text-xxs tracking-wide cursor-pointer ${darkMode ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' : 'bg-rose-50 text-rose-600 border-rose-200'}`}><RotateCcw className="w-3 h-3"/> Reset Form</button>}
            </div>
            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="font-semibold text-xxs uppercase tracking-wider text-stone-500">Applicant Full Name</label>
                  <input type="text" className={`w-full border p-3 rounded-xl focus:outline-none transition-all ${darkMode ? 'bg-slate-950 border-amber-900/40 text-amber-100' : 'bg-amber-50/40 border-amber-200 text-stone-800'}`} placeholder="e.g. Alex Mercer" value={fullName} onChange={(e) => setFullName(e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <label className="font-semibold text-xxs uppercase tracking-wider text-stone-500">Target Role Objective</label>
                  <input type="text" className={`w-full border p-3 rounded-xl focus:outline-none transition-all ${darkMode ? 'bg-slate-950 border-amber-900/40 text-amber-100' : 'bg-amber-50/40 border-amber-200 text-stone-800'}`} placeholder="e.g. Senior Frontend Architect" value={targetRole} onChange={(e) => setTargetRole(e.target.value)} />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className={`space-y-1.5 p-4 rounded-xl border ${darkMode ? 'bg-amber-950/20 border-amber-900/40' : 'bg-amber-50/30 border-amber-100'}`}>
                  <label className="font-semibold text-xxs uppercase tracking-wider flex items-center gap-1"><FileText className="w-3.5 h-3.5 text-amber-600"/> Legacy Career Profile</label>
                  <p className="text-[10px] text-stone-400 mb-2">Hint: List parameters, engineering tools used, or clip structural history.</p>
                  <textarea rows={6} className={`w-full border p-3 rounded-xl focus:outline-none font-mono text-[11px] leading-relaxed ${darkMode ? 'bg-amber-950/60 border-amber-900 text-amber-200' : 'bg-white border-amber-200 text-stone-800'}`} placeholder="Frontend Dev at TechCorp. Managed core React components, optimizing layout loops..." value={careerHistory} onChange={(e) => setCareerHistory(e.target.value)} />
                </div>
                <div className={`space-y-1.5 p-4 rounded-xl border ${darkMode ? 'bg-amber-950/20 border-amber-900/40' : 'bg-amber-50/30 border-amber-100'}`}>
                  <label className="font-semibold text-xxs uppercase tracking-wider flex items-center gap-1"><Target className="w-3.5 h-3.5 text-amber-600"/> Job Description Target</label>
                  <p className="text-[10px] text-stone-400 mb-2">Hint: Paste complete requirements checklist from your target hiring pipe.</p>
                  <textarea rows={6} className={`w-full border p-3 rounded-xl focus:outline-none font-mono text-[11px] leading-relaxed ${darkMode ? 'bg-amber-950/60 border-amber-900 text-amber-200' : 'bg-white border-amber-200 text-stone-800'}`} placeholder="Seeking an engineer with deep runtime comprehension of cloud topologies..." value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} />
                </div>
              </div>
              <button type="button" onClick={handleGenerate} disabled={loading} className="w-full bg-amber-800 hover:bg-amber-700 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-all text-xxs uppercase tracking-widest disabled:bg-stone-300 dark:disabled:bg-amber-950/40">
          ...
          <div className={`border p-6 rounded-2xl shadow-sm space-y-5 ${darkMode ? 'bg-amber-900/10 border-amber-900/40' : 'bg-white border-amber-200'}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              <div className={`p-6 rounded-2xl border text-center relative overflow-hidden ${darkMode ? 'bg-amber-950 border-amber-800' : 'bg-amber-50/80 border-amber-200'}`}>
                <BarChart3 className="w-6 h-6 text-amber-700 mx-auto mb-2"/>
              ...
              <div className={`p-6 rounded-2xl border ${darkMode ? 'bg-amber-950 border-amber-800' : 'bg-stone-900 text-stone-100 border-stone-950'}`}>
                <h4 className="text-amber-400 font-extrabold uppercase tracking-widest text-[10px] flex items-center gap-1.5 border-b pb-1.5 border-amber-500/20 mb-3"><AlertTriangle className="w-4 h-4 text-amber-400"/> Critical Keyword Gaps Isolate</h4>
                <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto pr-1">
                  {results.missing_skills?.map((s: string, i: number) => <span key={i} className="bg-amber-400/20 text-amber-300 font-bold px-2.5 py-1 rounded-lg border border-amber-400/30 text-[10px] tracking-wide shadow-sm">{s}</span>) || <span className="text-stone-400 italic text-[10px]">None</span>}
                </div>
              </div>
              <div className={`p-6 rounded-2xl border ${darkMode ? 'bg-amber-950 border-amber-800' : 'bg-[#F5EFE6] border-amber-200/80 text-stone-900'}`}>
                <h4 className="text-amber-900 dark:text-amber-400 font-extrabold uppercase tracking-widest text-[10px] flex items-center gap-1.5 border-b pb-1.5 border-amber-700/20 mb-3"><Target className="w-4 h-4 text-amber-700"/> Optimization Strategy Advice</h4>
                <ul className="space-y-2 text-[10px] font-medium leading-relaxed max-h-32 overflow-y-auto pr-1 list-none pl-0">{results.tailoring_tips?.map((t: string, i: number) => <li key={i} className="flex items-start gap-2 border-b border-dashed border-amber-700/10 pb-1 last:border-0"><CheckCircle2 className="w-3.5 h-3.5 text-amber-700 shrink-0 mt-0.5"/> <span>{t}</span></li>)}</ul>
              </div>
            </div>
          </div>
        )}
        ...
        {tab === 'updated_resume' && results && (
          <div className={`border p-6 rounded-2xl shadow-sm space-y-5 ${darkMode ? 'bg-amber-900/10 border-amber-900/40' : 'bg-white border-amber-200'}`}>
            ...
            <div className={`border p-6 rounded-xl space-y-4 max-h-[500px] overflow-y-auto shadow-inner leading-relaxed ${darkMode ? 'border-amber-900/40 bg-amber-950/80' : 'border-amber-200 bg-amber-50/40'}`}>
              <h2 className={`text-xl font-extrabold tracking-tight border-b pb-2 ${darkMode ? 'text-white border-amber-800' : 'text-slate-900 border-amber-200'}`}>{results.resume?.full_name}</h2>
              <p className="text-xs font-medium tracking-wide text-slate-400 dark:text-amber-200/70">{results.resume?.professional_summary}</p>
              <div className="flex flex-wrap gap-1.5 pt-1">{results.resume?.skills?.map((s: string, i: number) => <span key={i} className={`font-bold px-2 py-0.5 rounded-md border text-[10px] tracking-wide ${darkMode ? 'bg-amber-500/10 text-amber-300 border-amber-500/20' : 'bg-amber-50 text-amber-800 border-amber-200'}`}>{s}</span>)}</div>
              ...
              <div className="space-y-4">
                ...
                {results.hr_interview?.map((item: any, i: number) => (
                  <div key={i} className={`p-4 rounded-xl border space-y-2 transition-all ${darkMode ? 'bg-amber-950/60 border-amber-900/40 hover:border-amber-800' : 'bg-amber-50/40 border-amber-200 hover:border-amber-300'}`}>
                    ...
                    <div className="text-[11px] pl-5 leading-relaxed text-amber-200/70 dark:text-stone-600 font-medium"><span className="font-bold text-amber-800 dark:text-amber-400">Response Logic:</span> {item.response}</div>
                  </div>
                ))}
              </div>
              <div className="space-y-4">
                ...
                {results.technical_interview?.map((item: any, i: number) => (
                  <div key={i} className={`p-4 rounded-xl border space-y-2 transition-all ${darkMode ? 'bg-amber-950/60 border-amber-900/40 hover:border-amber-800' : 'bg-amber-50/40 border-amber-200 hover:border-amber-300'}`}>
                    ...
                    <div className="text-[11px] pl-5 leading-relaxed text-amber-200/70 dark:text-stone-600 font-medium"><span className="font-bold text-amber-800 dark:text-amber-400">Technical Strategy:</span> {item.response}</div>
                  </div>
                ))}
              </div>
            </div>
