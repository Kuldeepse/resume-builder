      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {tab === 'builder' && (
          <div className={`border p-6 rounded-2xl shadow-sm space-y-6 ${darkMode ? 'bg-slate-900/40 border-slate-800/80 shadow-2xl' : 'bg-white border-slate-200'}`}>
            <div className={`flex justify-between items-center border-b pb-3 ${darkMode ? 'border-slate-800/60' : 'border-slate-100'}`}>
              ...
              {results && <button type="button" onClick={() => { setFullName(''); setTargetRole(''); setCareerHistory(''); setJobDescription(''); setResults(null); setTab('builder'); }} className={`flex items-center gap-1 font-bold px-3 py-1 rounded-xl border transition-all text-xxs tracking-wide cursor-pointer ${darkMode ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' : 'bg-rose-50 text-rose-600 border-rose-200'}`}><RotateCcw className="w-3 h-3"/> Purge Matrix</button>}
            </div>
            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="font-semibold text-xxs uppercase tracking-wider text-slate-400 dark:text-slate-500">Applicant Full Name</label>
                  <input type="text" className={`w-full border p-3 rounded-xl focus:outline-none transition-all ${darkMode ? 'bg-slate-950 border-slate-800 text-slate-200' : 'bg-slate-50 border-slate-200 text-slate-800'}`} placeholder="e.g. Alex Mercer" value={fullName} onChange={(e) => setFullName(e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <label className="font-semibold text-xxs uppercase tracking-wider text-slate-400 dark:text-slate-500">Target Role Objective</label>
                  <input type="text" className={`w-full border p-3 rounded-xl focus:outline-none transition-all ${darkMode ? 'bg-slate-950 border-slate-800 text-slate-200' : 'bg-slate-50 border-slate-200 text-slate-800'}`} placeholder="e.g. Senior Frontend Architect" value={targetRole} onChange={(e) => setTargetRole(e.target.value)} />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className={`space-y-1.5 p-4 rounded-xl border ${darkMode ? 'bg-slate-950/30 border-slate-800/40' : 'bg-slate-50/50 border-slate-100'}`}>
                  <label className="font-semibold text-xxs uppercase tracking-wider flex items-center gap-1"><FileText className="w-3.5 h-3.5 text-indigo-500"/> Legacy Career Profile</label>
                  <p className="text-[10px] text-slate-400 mb-2">Hint: List parameters, technology stack tools used, or clip history.</p>
                  <textarea rows={6} className={`w-full border p-3 rounded-xl focus:outline-none font-mono text-[11px] leading-relaxed ${darkMode ? 'bg-slate-950 border-slate-800 text-slate-300' : 'bg-white border-slate-200 text-slate-800'}`} placeholder="Frontend Dev at TechCorp. Managed React architecture, optimizing layout loops..." value={careerHistory} onChange={(e) => setCareerHistory(e.target.value)} />
                </div>
                <div className={`space-y-1.5 p-4 rounded-xl border ${darkMode ? 'bg-slate-950/30 border-slate-800/40' : 'bg-slate-50/50 border-slate-100'}`}>
                  <label className="font-semibold text-xxs uppercase tracking-wider flex items-center gap-1"><Target className="w-3.5 h-3.5 text-emerald-500"/> Job Description Target</label>
                  ...
                  <textarea rows={6} className={`w-full border p-3 rounded-xl focus:outline-none font-mono text-[11px] leading-relaxed ${darkMode ? 'bg-slate-950 border-slate-800 text-slate-300' : 'bg-white border-slate-200 text-slate-800'}`} placeholder="Seeking an engineer with deep runtime comprehension of cloud topologies..." value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} />
                </div>
              </div>
              <button type="button" onClick={handleGenerate} disabled={loading} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-all text-xxs uppercase tracking-widest disabled:bg-slate-300 dark:disabled:bg-slate-800">
        ...
        {tab === 'validation' && results && (
          <div className={`border p-6 rounded-2xl shadow-sm space-y-5 ${darkMode ? 'bg-slate-900/40 border-slate-800/80' : 'bg-white border-slate-200'}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className={`p-5 rounded-2xl text-center border relative overflow-hidden ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-100 border-slate-200'}`}>
                <BarChart3 className="w-6 h-6 text-indigo-500 mx-auto mb-1.5"/>
                <h4 className="font-bold uppercase tracking-widest text-[9px] text-slate-400 dark:text-slate-500">ATS Alignment Matrix</h4>
                <div className={`text-4xl font-black mt-1 ${darkMode ? 'text-white' : 'text-indigo-600'}`}>{results.match_score}%</div>
              </div>
              <div className={`p-5 rounded-2xl border ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-100 border-slate-200'}`}>
              ...
              <div className={`p-5 rounded-2xl border ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-100 border-slate-200'}`}>
                ...
                <ul className="space-y-1.5 text-[10px] leading-relaxed max-h-24 overflow-y-auto pr-1 list-none text-slate-400 dark:text-slate-600">{results.tailoring_tips?.map((t: string, i: number) => <li key={i} className="flex items-start gap-1.5"><CheckCircle2 className="w-3 h-3 text-indigo-500 shrink-0 mt-0.5"/> <span>{t}</span></li>)}</ul>
              </div>
            </div>
          </div>
        )}
        ...
        {tab === 'updated_resume' && results && (
          <div className={`border p-6 rounded-2xl shadow-sm space-y-5 ${darkMode ? 'bg-slate-900/40 border-slate-800/80' : 'bg-white border-slate-200'}`}>
            ...
            <div className={`border p-6 rounded-xl space-y-4 max-h-[500px] overflow-y-auto shadow-inner leading-relaxed ${darkMode ? 'border-slate-800 bg-slate-950/80' : 'border-slate-200 bg-slate-100'}`}>
              ...
              <p className="text-xs font-normal tracking-wide text-slate-400 dark:text-slate-600">{results.resume?.professional_summary}</p>
              ...
              <div className="space-y-4 pt-3">
                {results.resume?.experience?.map((exp: any, i: number) => (
                  <div key={i} className="space-y-1.5 border-l-2 border-indigo-500/50 pl-4 relative">
                    ...
                    <ul className="list-none space-y-1 text-[10px] pl-0 text-slate-400 dark:text-slate-600">{exp.bullet_points?.map((b: string, idx: number) => <li key={idx} className="flex items-start gap-2">⚡ <span>{b}</span></li>)}</ul>
                  </div>
                ))}
              </div>
            </div>
        ...
        {tab === 'prep' && results && (
          <div className={`border p-6 rounded-2xl shadow-sm space-y-5 ${darkMode ? 'bg-slate-900/40 border-slate-800/80' : 'bg-white border-slate-200'}`}>
            <div className={`p-4 rounded-xl text-white flex justify-between items-center shadow-md ${darkMode ? 'bg-slate-950 border border-slate-800' : 'bg-slate-800'}`}>
              ...
              <button onClick={handleCopyLink} className="bg-indigo-600 text-white px-3 py-1.5 rounded-xl font-bold cursor-pointer text-[10px] uppercase tracking-wider transition-all hover:bg-indigo-500 shadow-md border border-indigo-500/40">{copied ? "Link Copied!" : "Copy Public PDF Link"}</button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-h-[500px] overflow-y-auto pr-1">
              <div className="space-y-4">
                ...
                {results.hr_interview?.map((item: any, i: number) => (
                  <div key={i} className={`p-4 rounded-xl border space-y-2 transition-all ${darkMode ? 'bg-slate-950/60 border-slate-800/60 hover:border-slate-700/80' : 'bg-slate-50 border-slate-200 hover:border-slate-300'}`}>
                    ...
                    <div className="text-[11px] pl-5 leading-relaxed text-slate-400 dark:text-slate-600"><span className="font-semibold text-indigo-600 dark:text-indigo-400">Response Logic:</span> {item.response}</div>
                  </div>
                ))}
              </div>
              <div className="space-y-4">
                ...
                {results.technical_interview?.map((item: any, i: number) => (
                  <div key={i} className={`p-4 rounded-xl border space-y-2 transition-all ${darkMode ? 'bg-slate-950/60 border-slate-800/60 hover:border-slate-700/80' : 'bg-slate-50 border-slate-200 hover:border-slate-300'}`}>
                    ...
                    <div className="text-[11px] pl-5 leading-relaxed text-slate-400 dark:text-slate-600"><span className="font-semibold text-indigo-600 dark:text-indigo-400">Technical Strategy:</span> {item.response}</div>
                  </div>
                ))}
              </div>
            </div>
