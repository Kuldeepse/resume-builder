...
const ARBEITNOW_UK_API = 'https://www.arbeitnow.com/api/job-board-api';
  ...
  try {
    ...
    return NextResponse.json({
      ...
      best_match_summary: scored.length
        ? `Found ${scored.length} current vacancies from a free public job feed and ranked them using CogniTwist's deterministic CV-to-job fit engine.`
        : `No sufficiently relevant vacancies were found in the latest feed for “${targetRole}”. Try a broader title such as Product Manager, Programme Manager, Delivery Manager or Technical Project Manager.`,
      search_mode: 'free_public_feed',
      source: 'Arbeitnow',
    });
  } catch (error) {
    return NextResponse.json({
      detail: error instanceof Error ? `Free job search is temporarily unavailable: ${error.message}` : 'Free job search is temporarily unavailable.',
    }, { status: 502 });
  }
}
