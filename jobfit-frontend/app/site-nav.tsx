        ...
        {items.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-extrabold ${active ? 'bg-[var(--accent)] text-white' : 'text-[var(--ink-soft)]'}`}
              aria-current={active ? 'page' : undefined}
            >
