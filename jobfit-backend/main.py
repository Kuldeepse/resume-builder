...
export default function Home() {
  ...
  const [tab, setTab] = useState<'builder' | 'agent' | 'validation' | 'updated_resume' | 'prep' | 'job_search'>('builder');
  ...
  const handleRunAgent = async () => {
    if (!targetRole.trim()) {
      return alert('Please enter Target Role Objective before running the full workflow agent.');
    }

    if (!careerHistory.trim() && !agentFile) {
      return alert('Please provide career history or attach a CV for the full workflow agent.');
    }
    ...
    const stages = [
      ...
      'Searching matching live jobs...',
      'Selecting the strongest fit...',
      'Tailoring your resume...',
      'Building interview prep...',
    ];
    setAgentStage(stages[0]);
    let stageIndex = 0;
    const stageTimer = window.setInterval(() => {
      stageIndex = Math.min(stageIndex + 1, stages.length - 1);
      setAgentStage(stages[stageIndex]);
    }, 1600);

    const formData = new FormData();
    formData.append('goal', 'full_workflow');
    formData.append('full_name', fullName.trim());
    formData.append('target_role', targetRole.trim());
    formData.append('location_city', searchCity.trim() || 'Remote');
    formData.append('resume_skills', searchRoleSkills.trim());
    formData.append('career_history', careerHistory.trim());
    formData.append('job_description', jobDescription.trim());
    formData.append('linkedin_profile', linkedin.trim());
    ...
    if (agentFile) {
      formData.append('resume_file', agentFile);
    }

    try {
      const res = await fetch('https://resume-builder-backend-ph7b.onrender.com/run-agent', {
        method: 'POST',
        body: formData,
      });
      ...
      if (!res.ok) {
        throw new Error(data?.detail || 'Full workflow agent failed.');
      }

      setAgentResults(data);
      setTab('agent');
    } catch (error: any) {
      alert(error?.message || 'RoleCraft AI agent could not complete the workflow right now.');
    } finally {
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className={`flex flex-wrap border p-1.5 gap-1.5 backdrop-blur-md rounded-2xl shadow-sm overflow-x-auto ${panelTheme}`}>
          ...
          <button
            ...
            className={`px-4 py-2 font-bold rounded-xl flex items-center gap-1.5 transition-all duration-200 text-xxs tracking-wide ${
              tab === 'agent' ? 'bg-amber-900 text-white shadow-md scale-[1.01]' : darkMode ? 'text-stone-300 hover:bg-stone-800' : 'text-stone-700 hover:bg-amber-50'
            }`}
          >
            <PlayCircle className="w-3.5 h-3.5" /> Full Workflow Agent
          </button>
        ...
        {tab === 'builder' && (
          <div className={`border p-6 rounded-2xl shadow-sm space-y-6 ${panelTheme}`}>
            ...
            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              ...
              <div className={`rounded-xl border p-4 space-y-4 ${darkMode ? 'border-stone-700 bg-stone-950/30' : 'border-[#e6d4bc] bg-[#fff8ef]'}`}>
                <div className="flex flex-col gap-1">
                  <div className={`text-[11px] uppercase tracking-[0.14em] font-black ${sectionLabelTheme}`}>Full Workflow Agent</div>
                  <p className={`text-xs leading-6 ${mutedTextTheme}`}>
                    Let the agent search matching roles, tailor your resume for the strongest match, and build an interview pack in one run.
                  </p>
                </div>

                <div className={`border-2 border-dashed rounded-xl p-4 text-center transition-all ${agentFile ? 'border-green-500 bg-green-500/5' : darkMode ? 'border-stone-700 bg-stone-950/40' : 'border-[#d9c8b4] bg-white/70'}`}>
                  <input type="file" id="agent-file-picker" accept=".pdf,.docx" className="hidden" onChange={(e) => setAgentFile(e.target.files?.[0] || null)} />
                  <label htmlFor="agent-file-picker" className="cursor-pointer block space-y-2">
                    <FileText className={`w-7 h-7 mx-auto ${agentFile ? 'text-green-500' : darkMode ? 'text-stone-400' : 'text-stone-500'}`} />
                    <div className="text-xxs font-bold uppercase tracking-wide">
                      {agentFile ? `Agent CV Attached: ${agentFile.name}` : 'Attach CV for the full workflow agent'}
                    </div>
                  </label>
                  {agentFile && (
                    <button type="button" onClick={() => setAgentFile(null)} className="mt-2 text-[10px] font-bold text-rose-600 hover:underline">
                      Remove Agent CV
                    </button>
                  )}
                </div>

                <button type="button" onClick={handleRunAgent} disabled={agentLoading} className="w-full bg-stone-900 hover:bg-stone-800 dark:bg-amber-700 dark:hover:bg-amber-600 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 text-xxs uppercase tracking-widest disabled:opacity-60">
                  {agentLoading ? (
                    <>
                      <RefreshCw className="animate-spin w-4 h-4 text-amber-200" /> Running Full Workflow...
                    </>
                  ) : (
                    <>
                      <PlayCircle className="w-4 h-4 text-amber-300" /> Run Full Workflow Agent
                    </>
                  )}
                </button>
                {agentLoading && (
                  <div className={`text-xs font-medium ${darkMode ? 'text-stone-300' : 'text-stone-700'}`}>
                    {agentStage}
                  </div>
                )}
              </div>
        ...
        {tab === 'agent' && (
          <div className={`border p-6 rounded-2xl shadow-sm space-y-6 ${panelTheme}`}>
            <div className={`border-b pb-3 ${darkMode ? 'border-stone-800' : 'border-stone-200'}`}>
              <h2 className={`text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 ${sectionLabelTheme}`}>
                <PlayCircle className="w-4 h-4" /> Full Workflow Agent
              </h2>
            </div>

            {agentResults ? (
              <div className="space-y-6">
                <div className={`rounded-xl border p-4 ${darkMode ? 'border-stone-800 bg-stone-950/40' : 'border-stone-200 bg-white/80'}`}>
                  <div className="flex flex-col gap-2">
                    <div className={`text-[11px] uppercase tracking-[0.14em] font-black ${sectionLabelTheme}`}>Agent Summary</div>
                    <p className="text-sm leading-7">{agentResults.agent_summary}</p>
                  </div>
                </div>

                {agentResults.agent_trace?.length > 0 && (
                  <div className={`rounded-xl border p-4 ${darkMode ? 'border-stone-800 bg-stone-950/40' : 'border-stone-200 bg-white/80'}`}>
                    <div className={`text-[11px] uppercase tracking-[0.14em] font-black mb-3 ${sectionLabelTheme}`}>Agent Trace</div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      {agentResults.agent_trace.map((step: string, index: number) => (
                        <div key={index} className={`rounded-lg border p-3 text-xs leading-6 ${darkMode ? 'border-stone-800 bg-stone-900/40' : 'border-stone-200 bg-stone-50/60'}`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  <div className={`rounded-xl border p-4 space-y-3 ${darkMode ? 'border-stone-800 bg-stone-950/40' : 'border-stone-200 bg-white/80'}`}>
                    <div className={`text-[11px] uppercase tracking-[0.14em] font-black ${sectionLabelTheme}`}>Candidate Profile</div>
                    <div className="space-y-2 text-xs leading-6">
                      <div><span className="font-bold">Target:</span> {agentResults.candidate_profile?.target_role || targetRole}</div>
                      <div><span className="font-bold">Location:</span> {agentResults.candidate_profile?.location_preference || searchCity || 'Remote'}</div>
                      {agentResults.candidate_profile?.seniority && <div><span className="font-bold">Seniority:</span> {agentResults.candidate_profile.seniority}</div>}
                      {agentResults.candidate_profile?.summary && <p>{agentResults.candidate_profile.summary}</p>}
                    </div>
                  </div>

                  <div className={`rounded-xl border p-4 space-y-3 ${darkMode ? 'border-stone-800 bg-stone-950/40' : 'border-stone-200 bg-white/80'}`}>
                    <div className={`text-[11px] uppercase tracking-[0.14em] font-black ${sectionLabelTheme}`}>Selected Role</div>
                    {agentResults.selected_job?.title ? (
                      <div className="space-y-2 text-xs leading-6">
                        <div className="font-bold text-sm">{agentResults.selected_job.title}</div>
                        <div>{agentResults.selected_job.company}</div>
                        <div>{agentResults.selected_job.location}</div>
                        <div>{agentResults.selected_job.salary}</div>
                      </div>
                    ) : (
                      <p className={`text-xs ${mutedTextTheme}`}>The agent will show the strongest matched role here after a full run.</p>
                    )}
                  </div>

                  <div className={`rounded-xl border p-4 space-y-3 ${darkMode ? 'border-stone-800 bg-stone-950/40' : 'border-stone-200 bg-white/80'}`}>
                    <div className={`text-[11px] uppercase tracking-[0.14em] font-black ${sectionLabelTheme}`}>Workflow Output</div>
                    <div className="space-y-2 text-xs leading-6">
                      <div><span className="font-bold">Resume Score:</span> {agentResults.resume_analysis?.match_score ?? '--'}%</div>
                      <div><span className="font-bold">Questions:</span> {agentResults.interview_pack?.interview_questions?.length ?? 0}</div>
                      <div><span className="font-bold">Jobs Ranked:</span> {agentResults.top_jobs?.length ?? 0}</div>
                    </div>
                  </div>
                </div>

                {agentResults.top_jobs?.length > 0 && (
                  <div className="space-y-3">
                    <div className={`text-[11px] uppercase tracking-[0.14em] font-black ${sectionLabelTheme}`}>Top Matched Jobs</div>
                    <div className="grid grid-cols-1 gap-3">
                      {agentResults.top_jobs.slice(0, 5).map((job: any, index: number) => (
                        <div key={index} className={`rounded-xl border p-4 flex flex-col gap-2 ${darkMode ? 'border-stone-800 bg-stone-950/40' : 'border-stone-200 bg-white/80'}`}>
                          <div className="font-bold text-sm">{job.title}</div>
                          <div className={`text-xs ${mutedTextTheme}`}>{job.company} · {job.location}</div>
                          <div className="text-xs">{job.salary}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                  <div className={`rounded-xl border p-4 space-y-4 ${darkMode ? 'border-stone-800 bg-stone-950/40' : 'border-stone-200 bg-white/80'}`}>
                    <div className={`text-[11px] uppercase tracking-[0.14em] font-black ${sectionLabelTheme}`}>Tailored Resume Snapshot</div>
                    <div className="space-y-3">
                      <div className="font-extrabold text-lg">{agentResults.tailored_resume?.full_name || fullName || 'Candidate'}</div>
                      <div className={`text-[11px] uppercase tracking-[0.14em] font-bold ${darkMode ? 'text-stone-300' : 'text-stone-700'}`}>
                        {agentResults.tailored_resume?.headline || `Tailored Resume Draft for ${targetRole || 'Target Role'}`}
                      </div>
                      <p className="text-sm leading-7">{agentResults.tailored_resume?.professional_summary}</p>
                    </div>
                  </div>

                  <div className={`rounded-xl border p-4 space-y-4 ${darkMode ? 'border-stone-800 bg-stone-950/40' : 'border-stone-200 bg-white/80'}`}>
                    <div className={`text-[11px] uppercase tracking-[0.14em] font-black ${sectionLabelTheme}`}>Interview Pack</div>
                    <div className="space-y-3">
                      {agentResults.interview_pack?.tell_me_about_yourself && (
                        <p className="text-sm leading-7">{agentResults.interview_pack.tell_me_about_yourself}</p>
                      )}
                      {agentResults.interview_pack?.interview_questions?.slice(0, 3).map((item: any, i: number) => (
                        <div key={i} className={`rounded-lg border p-3 ${darkMode ? 'border-stone-800 bg-stone-900/40' : 'border-stone-200 bg-stone-50/50'}`}>
                          <div className="font-bold text-xs">Q{i + 1}: {item.question}</div>
                          <div className="text-[11px] mt-2 whitespace-pre-wrap">{item.response}</div>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div className={`rounded-xl border p-6 ${darkMode ? 'border-stone-800 bg-stone-950/40' : 'border-stone-200 bg-white/80'}`}>
                <p className={`text-sm leading-7 ${mutedTextTheme}`}>
                  Run the Full Workflow Agent from the Pipeline Builder tab to search matching roles, tailor the resume for the strongest fit, and assemble an interview prep pack in one pass.
                </p>
              </div>
            )}
          </div>
